...
export default {
    ...
    methods:{
        ...
        let k;
        let _id=this._id;
        k=await database.getDocument(config.website_db,config.gallery,this.id,[Query.select(["title_hu","title_en","title_rs"])]);
        ...
        let l= await database.listDocuments(config.website_db, config.album_images,[Query.equal("gallery",this.id)]);
