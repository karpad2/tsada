<template>
  <!-- Static shell always present for SSR/crawlers; video enhances on client -->
  <div class="hero-shell flex flex-wrap w-full mb-10 min-h-[280px] md:min-h-[360px] -mt-[5px]">
    <component
      :is="videoComponent"
      v-if="loaded && videoComponent"
      :src="integrated_video"
      :options="options"
      class="hero-video flex flex-wrap w-full min-h-[280px] md:min-h-[360px]"
    ...
    >
      ...
      <section class="text-gray-600 body-font relative z-10 w-full" id="about">
        <div
          class="container mx-auto text-white flex px-5 py-8 md:py-12 md:flex-row flex-col items-center"
        >
          <div
            class="glass-panel rounded-3xl p-6 md:p-10 max-w-xl lg:flex-grow md:w-1/2 flex flex-col md:items-start md:text-left items-center text-center shadow-2xl"
          >
            <h1
            ...
            <p
              v-if="showParagraph"
              class="mb-6 leading-relaxed items-center text-white/90 text-base md:text-lg"
            >
              {{ $t('whycomehere') }}
    ...
    <div
      v-else
      class="hero-static relative w-full min-h-[280px] md:min-h-[360px] flex items-center"
    >
      <div class="hero-overlay absolute inset-0 pointer-events-none"></div>
      <section class="text-gray-600 body-font relative z-10 w-full" id="about">
        <div
        ...
        >
          ...
          >
            ...
            <p class="mb-6 leading-relaxed items-center text-white/90 text-base md:text-lg">
              {{ $t('whycomehere') }}
              <span class="font-semibold text-sky-300">{{ $t('msc') }}</span>.
            </p>
...
export default {
  ...
  mounted() {
    ...
    import('vue-responsive-video-background-player')
      .then((mod) => {
        this.videoComponent = mod.default
      })
      .catch(() => {})
