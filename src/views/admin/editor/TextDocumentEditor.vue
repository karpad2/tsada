<template>
    <div class="relative mb-4 container  px-5  mx-auto bg-white" >
        ...
        <div>
            <v-file-input @change="file_upload" v-model="file_link"  accept=".pdf" :label="$t('fileupload')"></v-file-input>
    ...
    async getMD()
        {
            ...
            let k= await database.listDocuments(config.website_db, config.text_documents,[Query.equal("$id",this.$route.params.id)]);
...
/*
    ...
    {   
        ...
        config.text_documents, // collectionId
    ...
    async delete_content()
    {
        ...
        try{
        let k= await database.deleteDocument(config.website_db, config.text_documents,this.$route.params.id);  
        }
        catch(ex)
        {
            console.warn(ex)
