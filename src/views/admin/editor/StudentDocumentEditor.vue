    ...
    async getMD()
        {
            ...
            let k= await database.listDocuments(config.website_db, config.st_documents,[Query.equal("$id",this.$route.params.id)]);
...
/*
    ...
    async delete_content()
    {
        ...
        try{
        let k= await database.deleteDocument(config.website_db, config.st_documents,this.$route.params.id);  
        }
        catch(ex)
        {
            console.warn(ex)
